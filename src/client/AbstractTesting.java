 ...
 * 
 * The command+flags parameter contains upper case characters to set a command
 * and lower case parameters to set a flag.
 * 
 * This file only provides the framework for a scripted testing. Subclasses are
 * expected to implement individual commands in the the processLine method.
 */
public abstract class AbstractTesting {
	...
	/**
	 * This is the main method that is supposed to be run by the client. For
	 * simplicity, many exceptions are not handled and thrown to the client.
	 * 
	 * @throws Exception
	 */
	final public void run() throws Exception {
		...
		while (in.ready()) {
			...
			try {
				...
				System.err.println(String.format("Exception caused by script line %d (%s): %s", currentLine, line,
						iae.getMessage()));
				throw iae;
			}
	 ...
	 * 
	 * @throws ApplicationException     Any exception thrown by the
	 *                                  application-under-test is should be wrapped
	 *                                  into this exception. These exceptions are
	 *                                  accounted for when checking for "expected
	 *                                  exception" flag in the script
	 * @throws IllegalArgumentException This exception must be thrown to indicate
	 *                                  that the script line is unexpected. It will
	 *                                  cancel further processing of the script.
	 * 
	 */
	protected abstract void processLine(String name, String cmd, List<Character> flags,
			StringTokenizer scriptLineTokens) throws ApplicationException;
